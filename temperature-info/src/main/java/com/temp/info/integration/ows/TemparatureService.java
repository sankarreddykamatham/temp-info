...
public class TemparatureService {
	...
	private static final Logger logger = LoggerFactory.getLogger(TemparatureService.class);

	private final RestTemplate restTemplate;

	private final String apiKey;

	public TemparatureService(RestTemplateBuilder restTemplateBuilder,
			TemparatureAppProperties properties) {
		this.restTemplate = restTemplateBuilder.build();
		this.apiKey = properties.getApi().getKey();
	}

	@Cacheable("weather")
	public Temparature getWeather(String country, String city) {
		logger.info("Requesting current weather for {}/{}", country, city);
		URI url = new UriTemplate(WEATHER_URL).expand(city, country, this.apiKey);
		return invoke(url, Temparature.class);
	}

	@Cacheable("forecast")
	public TemparatureForecast getWeatherForecast(String country, String city) {
		...
		return invoke(url, TemparatureForecast.class);
	}
